...
mode_descriptions = {
    ...
    "Agent": [
        "OpenAI Agents SDK와 Amazon Bedrock OpenAI 모델을 활용한 Agent입니다."
    ],
    "이미지 분석": [
...
with st.sidebar:
    ...
    st.markdown(
        "OpenAI Agents SDK와 Agent Skills를 이용하여 효과적인 Agent를 구현합니다." 
    ...
    agent_tools = ["current_time", "file_read", "file_write", "http_request"]
    default_agent_tool_selections = ["current_time", "file_read", "file_write"]
    ...
    default_agent_tool_selections = (
        ...
        or default_agent_tool_selections
    )
    agent_tool_selections = {
        tool: tool in default_agent_tool_selections for tool in agent_tools
    }
    ...
    if mode=="Agent" or mode=="Agent (Chat)":
        ...
        builtin_tool_selections = {}
        logger.info(f"default_agent_tool_selections: {default_agent_tool_selections}")

        with st.expander("Built-in Tool 옵션 선택", expanded=True):
            for tool in agent_tools:
                default_value = tool in default_agent_tool_selections
                builtin_tool_selections[tool] = st.checkbox(
                    tool, key=f"agent_tool_{tool}", value=default_value, disabled=False
                )

        selected_agent_tools = [
            name for name, is_selected in builtin_tool_selections.items() if is_selected
        ]
        logger.info(f"selected_agent_tools: {selected_agent_tools}")
        agent_tool_selections = builtin_tool_selections

        if selected_agent_tools != config.get("default_agent_tool_selections"):
            config["default_agent_tool_selections"] = selected_agent_tools
    ...
    selected_agent_tools = [
        tool for tool, is_selected in agent_tool_selections.items() if is_selected
    ]
        ...
        if mode == '일상적인 대화':
            ...
            if file_bytes is None:
                ...
                with st.status("thinking...", expanded=True, state="running") as status:
                    ...
                    artifacts_dir = openai_agent.ARTIFACTS_DIR
            ...
            with st.status("thinking...", expanded=True, state="running") as status:
                ...
                response, image_urls = asyncio.run(openai_agent.run_agent(
                    query=prompt,
                    optional_tool_names=selected_agent_tools,
                    mcp_server_names=mcp_servers,
                    skill_list=skill_list,
                    notification_queue=notification_queue,
                ))

        if chat.debug_mode == 'Disable' and mode != 'Agent':
           st.markdown(response)
